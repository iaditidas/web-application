  ...
  return (
    <div className="min-h-screen bg-white flex flex-col p-6 animate-fade-in">
      <header className="flex items-center gap-4 mb-8">
        <button onClick={onBack} className="p-2 -ml-2 text-stone-400 hover:text-orange-600">
          <Icons.History />
        </button>
        <div>
          <h2 className="text-xl font-black italic text-stone-900">TRACK ORDER</h2>
          <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">{currentOrder.id}</p>
        </div>
      </header>

      <div className="bg-orange-600 text-white rounded-3xl p-8 mb-12 shadow-2xl shadow-orange-100 relative overflow-hidden">
      ...
      <div className="bg-stone-50 rounded-2xl p-4 border border-stone-100 mb-8">
        ...
        {currentOrder.upiDetails && (
          <div className="flex justify-between items-center">
            <p className="text-xs font-bold text-stone-600">{currentOrder.upiDetails.provider}</p>
            <p className="text-xs font-black text-stone-900">{currentOrder.upiDetails.upiId}</p>
          </div>
        )}
        {!currentOrder.upiDetails && (
          <p className="text-xs font-bold text-stone-600">Cash on Delivery</p>
        )}
      </div>

      <div className="space-y-10 pl-4 relative">
        <div className="absolute left-4 top-2 bottom-2 w-0.5 bg-stone-100"></div>
        {steps.map((step, idx) => {
          ...
          return (
            <div key={step.status} className="flex gap-6 relative">
              <div className={`w-3 h-3 rounded-full z-10 mt-1.5 ring-4 ${
                isDone ? 'bg-orange-600 ring-orange-100' : 
                isCurrent ? 'bg-orange-600 animate-pulse ring-orange-100' : 
                'bg-stone-200 ring-transparent'
              }`} />
              <div>
                <h4 className={`font-black uppercase tracking-tight ${isPending ? 'text-stone-300' : 'text-stone-900'}`}>
                  {step.label}
                </h4>
                <p className={`text-xs font-medium ${isPending ? 'text-stone-200' : 'text-stone-500'}`}>
                  {step.desc}
                </p>
              </div>
            </div>
      ...
      <div className="mt-auto pt-8 flex flex-col items-center">
        <div className="bg-stone-50 w-full p-4 rounded-2xl border border-stone-100 flex items-center gap-4 mb-6">
          <div className="w-12 h-12 bg-stone-900 rounded-full flex items-center justify-center text-white">
            <Icons.User />
          </div>
          <div>
            <p className="text-xs font-bold text-stone-400 uppercase tracking-widest">Your Flash Rider</p>
            <p className="font-black uppercase text-stone-900">Deepak R.</p>
          </div>
          <button className="ml-auto bg-orange-600 p-3 rounded-xl text-white">
            <Icons.Flash />
          </button>
        </div>
